...
# Windows Host Architecture Deep Dive

## Use This Doc For

Use this document when you need to understand why the current Unix-oriented
runtime shape does not map cleanly to Windows hosting.

- For the higher-level scope and phased plan, see [Overview](./overview.md).
- For the final architecture decisions, see
  [Host Reference Design](./host-reference-design.md).
- For the implementation sequence, see
